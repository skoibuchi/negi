...
void* thread_packetcap( void* args ){
		signal(SIGUSR1, pcap_status);
...
//	}
	...
	if(pkt_queue.empty()){
		pthread_cancel(thread_cap);
	}
	if(stream0_queue.empty()){
		pthread_cancel(thread_stream0);
	}
	if(stream1_queue.empty()){
		pthread_cancel(thread_stream1);
	}
	/*
	if(thread_stream2.empty()){
		pthread_cancel(thread_stream2);
	}
	if(thread_stream3.empty()){
		pthread_cancel(thread_stream3);
	}
*/
	kill(getppid(), SIGTERM);
